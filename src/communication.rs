...
pub async fn buffer(stream: &mut tokio::net::TcpStream, buf: &mut [u8], bytes_start: usize, check_start: usize) -> Result<BufferResult, Box<dyn std::error::Error>> {
...
pub async fn buffer_until_complete(stream: &mut tokio::net::TcpStream, buf: &mut [u8]) -> Result<BufferResult, Box<dyn std::error::Error>> {
	...
	loop {
		...
		if bytes_checked == bytes_read {
			...
			buffer_result = buffer(stream, &mut buf[bytes_read..], bytes_read, bytes_checked).await?
		}
	}
